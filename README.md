...
Before reconstruction each projection is masked to the dosimeter.  Outside it ΔA is zero by construction, since those rays miss it entirely, so zeroing there is exact.  At the edge it is not: the dosimeter is solid urethane dyed throughout, so material near the surface darkens like any other, and there is no inert wall as there would be with a liquid in a container.  What dominates the edge is an optical artefact, rays grazing the curved surface and being refracted away, which carries no usable dose information and is the sharpest, highest-contrast feature in the frame.  On a real scan it reached ±1.08 OD against a bulk signal of about 0.1, and it survives whatever the rotation correction does.  Masking it discards a thin annulus of real dosimeter, which is a trade worth making while the beam is small and near the axis: on `scan_20260702_101800` it cut the reconstructed edge ring by 55x while leaving the interior unchanged.  Revisit it if dose is ever expected near the surface.

The edges are found per frame as the darkest sustained column either side of the axis, and the kept span covers both scans' edges since they need not agree.  If no edge is found in either frame that projection is left unmasked and the count is logged.  `dosimeter_seating.json` records how far the dosimeter orbited the axis in each session, which is the usual explanation when the rotation match fails.

Pre and post frames are paired by the rotation angle in the filename, not by sort order, so two scans that used a different starting angle or step size cannot be silently mismatched.  Pixels where either frame reads below `MIN_VALID_COUNTS` (10 counts) are masked to ΔA = 0: down there the log ratio is quantisation noise, and a frame reading zero would otherwise produce a spurious ~14 OD spike at the dosimeter's edge.
...
The encoding is recorded in `subtracted/encoding.json` as explicit scale and offset values, so scans written under any earlier version still decode correctly.  That sidecar also records **what was done** to the projections (`edge_masked`, `rotation_correction_deg`, `app_version`), because reconstruction reads whatever is already in `subtracted/`: a folder produced by an older version reconstructs silently and badly.  On a real scan an unmasked subtraction gave a 40 mm irradiated region where the same data masked gave 9 mm, with nothing in the output to say why.  A freshly captured scan needs nothing extra, since the post-irradiation session already subtracts.  For older scans, reconstruction warns when the projections predate edge masking, distinguishing that from the case where masking ran but found no edges (where re-running would not help).  **Recompute ΔA from pre/post images** in the Reconstruction panel handles the first case: it redoes the subtraction for the selected scan from its saved `pre/` and `post/` images, re-running the rotation check and the edge masking.  Nothing is re-captured; those folders are read, never written.  Reconstruct the scan again afterwards with **Force new volume** ticked, which the app sets for you.

This matters more than it sounds.  The subtraction has changed several times, and before this existed the only way to re-run it was to repeat the post scan, which is impossible once the dosimeter has been returned or has aged.  Scans were effectively frozen with whatever processing they were born with.  Scans with no sidecar at all are read with the original unsigned `OD_SCALE = 65535 / 4` scheme.
...
The dose centroid is auto-detected from the brightest 20 % of slices in the sample ROI, ranked by a high percentile rather than the slice mean (with a beam this small the mean of a slice is background, so the mean would rank slices by artefact content).  A guard band of `DOSE_EDGE_GUARD_FRAC` is excluded at each end of the sample region first: the dosimeter's end faces throw strong artefacts for the same reason its sides do, and without a guard they simply win.  On a real scan the first slice ran 1139 % above the column median, which made the brightest slices the edge ones and put the dose centroid on an end face.  Locating the beam within that map takes three steps, all needed because the beam is a small part of the slice:

The sanity check's axial and sagittal panels show the background-subtracted volume, scaled to the dosimeter interior, and mark the depth the dose peaks at.  They previously showed the raw volume scaled to the whole slice, where the dosimeter edge saturates the range: a real feature sat at 129 % of the display scale and was invisible, so the panel could not show what the dose was measured from.

1. The dosimeter's edge is excluded (`dosimeter_interior_mask`).  Whenever it fails to cancel it is the brightest thing in the slice, and being concentric with the rotation axis its centroid sits dead centre, which drags a whole-slice centroid to the middle however hard the map is thresholded.  What sits there is refraction rather than dose, so excluding it costs little.
2. The slowly varying, rotationally symmetric background is subtracted (`subtract_radial_background`).  A dosimeter darkens all over as its dye ages, which reconstructs as a smooth dome about the axis, and on a scan seven weeks after its pre-scan that background was several times the beam signal: the region found spanned the dosimeter, and only once it was removed did a localised column appear, holding position to 0.8 mm through 48 depth slices.

   Subtracting the plain azimuthal mean would remove it, but would also erase any beam centred on the rotation axis, since such a beam is itself rotationally symmetric and cannot be told from background by symmetry alone.  The separation is therefore made on **scale**, which holds wherever the beam sits: the background varies over the width of the dosimeter, a beam over its own width, so the radial profile is smoothed with a window `RADIAL_SMOOTH_BEAM_FACTOR` times the beam diameter before subtraction.  The profile is mirrored about r = 0 for the smoothing since it is even in radius, and a median is used so an off-axis beam cannot drag it.  Synthetic beams are recovered at full amplitude on the axis and out to 10 mm off it.

   A beam found within half a beam-width of the axis is still flagged (`irradiated_region_near_axis`), because that is where any residual suppression would act: its shape is reliable, its absolute level is a lower bound.
3. The threshold is taken at half the peak above local background, the usual FWHM convention.  Being a contrast ratio rather than an area, it needs no assumption about beam size: it locates a 3 mm beam and a 10 mm one equally well.  The peak is read at the 99.9th percentile, not the maximum, so a single hot pixel cannot define it.
...
The width and elongation of the region found are **reported, not judged**.  Earlier versions compared the width against an assumed beam diameter and warned when it did not match; that reference figure turned out to be unreliable, and a gate is only as good as the number behind it, so a real beam wider than assumed would have been flagged as an artefact.  Beam diameter is now entered per scan in the Reconstruction panel and recorded in `recon_config.json` for comparison, and nothing is gated on it.
...
The depth profile is taken as the **median** across the ROI, not the mean.  The dosimeter carries small bright inclusions, 59 of them on one real scan at a median of 0.54 mm across, scattered through its whole depth.  The ROI is only about 2 mm wide, so a single inclusion inside it lifts that slice enough to become the peak of the entire curve: on that scan a 0.90 mm inclusion at 38 mm depth did exactly that, and the real falloff was flattened beneath it.  A median ignores anything covering less than half the ROI, which every inclusion does.

The profile is then median filtered over `DOSE_DEPTH_MEDIAN_MM`, so a single bad slice cannot set the normalisation for the whole curve; anything a millimetre or wider passes through untouched.  The profile is also taken from four ROIs shifted by one ROI width, and the spread of the resulting peak depths is reported alongside it.  On a real scan the *shape* of the curve held to a correlation of 0.88 under those shifts while the *peak depth* moved over 11.9 mm, because the curve has several comparable maxima and which one is highest depends on exactly where the column sits.  A clean synthetic beam moves 0.1 mm by the same measure, so the spread distinguishes a curve whose peak means something from one whose peak does not.

The width of the peak is then measured as the contiguous run above half maximum and reported, because everything is scaled to the maximum: a narrow feature there sets the scale and flattens the broad component beneath it, and whether such a feature is dose or an inclusion is not something the app can decide.
