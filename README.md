...
2. Whatever is rotationally symmetric is subtracted (`subtract_radial_background`).  A dosimeter darkens all over as its dye ages, which reconstructs as a smooth dome about the axis, and on a scan seven weeks after its pre-scan that background was several times the beam signal: the region found spanned the dosimeter, and only once it was removed did a localised column appear, holding position to 0.8 mm through 48 depth slices.  Because it is a function of radius alone it subtracts cleanly; the cost is that a beam centred exactly on the axis would go with it.
